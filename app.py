...
if st.session_state.portfolio: 
    ...
    import yfinance as yf
           data = yf.download(tickers, period="7d",
...
try:
    # NSE Gainers/Losers
    nse_gainers = nse_top_gainers()
    nse_losers = nse_top_losers()
    st.markdown("### 🔼 NSE Top Gainers")
    st.dataframe(pd.DataFrame(nse_gainers))

    st.markdown("### 🔽 NSE Top Losers")
    st.dataframe(pd.DataFrame(nse_losers))

    # BSE Gainers/Losers
    b = BSE(update_codes=True)
    bse_gainers = b.topGainers()
    bse_losers = b.topLosers()
    st.markdown("### 🔼 BSE Top Gainers")
    st.dataframe(pd.DataFrame(bse_gainers))

    st.markdown("### 🔽 BSE Top Losers")
    st.dataframe(pd.DataFrame(bse_losers))
